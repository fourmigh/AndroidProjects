...
> **卸载重装会丢失本机绑定**：绑定信息存于应用私有目录（EncryptedSharedPreferences）与 Android Keystore，
> 卸载即被清除（**覆盖安装则保留**）。重装后需**重新绑定**；服务端旧凭证会成为孤儿，
> 可在「5. NFC 读取」页的绑定列表里解绑清理。
> （ECDSA 私钥丢失后无法找回，必须重新生成密钥对并重新绑定。）
...
NFC 出示时手机用哪种方式证明身份，由配置开关 `useEcdsa` 决定。

### HMAC 模式（对称，默认）

- 手机与服务端**共享同一把密钥** `nfcKey`
- 手机：`proof = HMAC-SHA256(nfcKey, challenge)`
- 服务端：用同一把 `nfcKey` 重算并比对
- **密钥由服务端生成**（`crypto.randomBytes(32)`），绑定时下发；手机存 EncryptedSharedPreferences
- 类比：双方各拿一把**相同的钥匙**
- 特点：实现简单、计算快；但密钥两边都有，服务端一旦泄露即可伪造所有用户

### ECDSA 模式（非对称签名）

- 手机持**私钥**，服务端持**公钥**
- 手机：`proof = ECDSA-SHA256(私钥, challenge)`
- 服务端：用公钥**验签**（只能验，不能伪造）
- **密钥对由手机生成**（Android Keystore，私钥不可导出）；绑定时只上传公钥
- 类比：手机有一枚**专属私章**，服务端有一张**只能验章真伪的卡**
- 特点：私钥不出手机，root 也难提取；服务端泄露也无法伪造；但签名慢、签名变长
...
|---|---|---|
| 密码学 | 对称 | 非对称 |
| **密钥生成方** | **服务端** | **手机（Android Keystore）** |
| 手机持有 | 共享密钥 `nfcKey` | 私钥（不可导出）|
| 服务端持有 | 同一把 `nfcKey` | 公钥 |
| 手机计算 | `HMAC(key, challenge)` | `ECDSA签名(私钥, challenge)` |
| 服务端验证 | 重算 HMAC 比对 | 公钥验签 |
| 手机被 root | 密钥可被提取 | 私钥不可导出 |
| 服务端泄露 | 可伪造 | 无法伪造 |
| 速度 / 签名长度 | 快 / 32B | 慢 / ~70–72B |
| 开关 | `useEcdsa=false`（默认）| `useEcdsa=true` |

### 如何切换

设置页「非对称签名 (ECDSA)」开关。**切换只影响新绑定**；已绑定用户按绑定时记录的模式验证，
仍可正常使用。要让某台设备切换到新模式，需**解绑后重新绑定**。
